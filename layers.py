...
def batch_norm(x, is_training = True):
    ...
    with tf.variable_scope("batch_norm"):
        return tf.keras.layers.BatchNormalization(momentum=0.90)(x, is_training)
...
def linear(x, units, norm_type='Batch', act_type='relu', is_training=True, name='linear'):
    with tf.variable_scope(name):
        x = tf.layers.dense(x, units=units, kernel_initializer=tf.contrib.layers.variance_scaling_initializer(), use_bias=False)
        if norm_type is not None:
            if norm_type=='Ins':
                x = instance_norm(x)
            elif norm_type=='Batch':
        ...
        if act_type is not None:
            if act_type=='relu':
                ...
                x = tf.nn.leaky_relu(x, 0.2, 'lrelu')
